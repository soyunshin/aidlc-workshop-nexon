...
export function OrderCard({ order }: OrderCardProps) {
  const time = new Date(order.ordered_at).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' });
    ...
    <div data-testid={`order-card-${order.id}`} style={{ border: '1px solid #e0e0e0', borderRadius: 8, padding: '1rem', marginBottom: '0.75rem' }}>
      ...
      <div style={{ fontSize: '0.9rem' }}>
        {order.items.map((item, idx) => (
          <span key={idx}>{item.item_name} ×{item.quantity}{idx < order.items.length - 1 ? ', ' : ''}</span>
        ))}
      </div>
